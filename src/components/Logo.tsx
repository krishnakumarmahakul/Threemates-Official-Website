...
export function Logo({ className, forceLight, size = "lg" }: { className?: string; forceLight?: boolean; size?: "sm" | "md" | "lg" }) {
  const containerClass = "relative overflow-hidden";
  const imgScale = "scale-[2.5]";
  const logoSrc = forceLight ? "/assets/describedLogo.png" : "/assets/darkDescribedlogo.png";
  ...
  return (
    <Link href="/" className={cn("relative flex items-center shrink-0 z-50 transition-transform duration-200 hover:scale-[0.98]", className)}>
      <div className={cn(containerClass, sizeClass)}>
        <Image
          src={logoSrc}
          alt="Threemates"
          width={400}
